"""Download historical bid data from EMI website."""
...
DATA_DIR = Path(__file__).parent.parent / "data" / "raw" / "bids"
...
async def download_file(client: httpx.AsyncClient, date: datetime) -> tuple[datetime, bool, float]:
    """Download bid file for given date. Returns (date, success, size_mb)."""
    filepath = DATA_DIR / f"{date:%Y%m%d}_Bids.csv"
    
    if filepath.exists():
        return date, True, 0.0
    
    url = f"{BASE_URL}/{date:%Y}/{date:%Y%m%d}_Bids.csv"
...
async def main():
    """Download all bid files in date range."""
    DATA_DIR.mkdir(parents=True, exist_ok=True)
    
    dates = [START_DATE + timedelta(days=i) for i in range((END_DATE - START_DATE).days + 1)]
    existing = {f.stem.split('_')[0] for f in DATA_DIR.glob('*_Bids.csv')}
    ...
    print(f"Total files: {len(list(DATA_DIR.glob('*_Bids.csv')))}")
